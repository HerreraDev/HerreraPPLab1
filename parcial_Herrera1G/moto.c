...
void mostrarMoto(eMoto moto, eColor colores[],int tamcolor, eTipo tipos[], int tamtipos)
{
    ...
    printf("\n%d    %10s          %10s          %10s      %d    \n",moto.id,moto.marca,descripcionColor,descripcionTipo,moto.cilindradas);
}
...
void mostrarMotos(eMoto motos[],int tammoto, eColor colores[],int tamcolor, eTipo tipos[], int tamtipos)
{
    ...
    for (int i=0; i< tammoto; i++)
    {
        ...
        {
            mostrarMoto(motos[i], colores, tamcolor,tipos,tamtipos);
            hayMotos = 1;
        }
    }
...
int altaMotos(int* proxId, eMoto lista[], int tammoto, eColor colores[], int tamcolor, eTipo tipos[], int tamtipos, int cilindrada[], int tamcilindrada)
{
    ...
    else
    {
        ...
        printf("Ingrese un rodado\n");
        elegirCilindrada(cilindrada,tamcilindrada);
        fflush(stdin);
        scanf("%d",&idCilindrada);
        switch(idCilindrada)
        {
        case 1:
...
void bajaMoto(eMoto motos[],int tammotos, eColor colores[],int tamcolor, eTipo tipos[], int tamtipos)
{
    ...
    {
        mostrarMoto(motos[indice],colores,tamcolor,tipos,tamtipos);
        fflush(stdin);
...
void modificarMoto(eMoto motos[], int tammotos,eColor colores[],int tamcolor, int cilindrada[], int tamcilindrada,eTipo tipos[],int tamtipos)
{
    ...
    else
    {
        ...
        {
            listarColores(colores,tamcolor);
            utn_getNumero(&nuevoColor,"Ingrese el id del color: \n","Error, color incorrecto \n",10000,10004,3);
        ...
        {
            elegirCilindrada(cilindrada,tamcilindrada);
            utn_getNumero(&idCilindrada,"Ingrese id de la cilindrada: \n","Error, no es valido el numero\n",1,5,3);
            switch(idCilindrada)
            {
            case 1:
